...
.result-box{background:#161b2e;border:1px solid #1e2a40;border-radius:10px;padding:16px;margin-bottom:12px}
</style>
...
if len(g1) > 1 and len(g2) > 1 and p1 != p2:
    ...
    if p_val < 0.05:
        st.markdown(f"<div style='background:#1a2a1a;border:1px solid #2a4a2a;border-radius:8px;padding:12px 16px;color:#4ade80;font-size:13px'>Différence statistiquement significative (p={p_val:.4f} &lt; 0.05) — Les prix entre {p1} et {p2} sont significativement différents.</div>", unsafe_allow_html=True)
    else:
        st.markdown(f"<div style='background:#1a1a2a;border:1px solid #2a2a4a;border-radius:8px;padding:12px 16px;color:#a78bfa;font-size:13px'>Pas de différence significative (p={p_val:.4f} &gt; 0.05) — On ne peut pas rejeter H0.</div>", unsafe_allow_html=True)
...
if len(groups) >= 2:
    ...
    if p_anova < 0.05:
        st.markdown("<div style='background:#1a2a1a;border:1px solid #2a4a2a;border-radius:8px;padding:12px 16px;color:#4ade80;font-size:13px'>ANOVA significative — Il existe des différences de prix significatives entre les marques.</div>", unsafe_allow_html=True)
    else:
        st.markdown("<div style='background:#1a1a2a;border:1px solid #2a2a4a;border-radius:8px;padding:12px 16px;color:#a78bfa;font-size:13px'>ANOVA non significative — Pas de différence notable entre les marques.</div>", unsafe_allow_html=True)
...
# ── SECTION 4 : Intervalles de confiance ──────────────────
...
for platform, grp in df.groupby('platform'):
    ...
    if len(prices) > 1:
        ...
        ci_data.append({'Plateforme': platform, 'Moyenne': prices.mean(),
                        'IC_bas': ci[0], 'IC_haut': ci[1]})
